...
export default function TemplatesPage() {
  ...
  useEffect(() => {
    templatesApi.getAll().then(setUserTemplates).catch(() => {})
  }, [])
  ...
  const filteredUser = userTemplates.filter(t => {
    const matchCat = activeCategory === 'All' || activeCategory === 'Custom' || t.category === activeCategory
    const matchQ = t.title.toLowerCase().includes(query.toLowerCase())
    return matchCat && matchQ
  })
    ...
    <AppLayout>
      <div className="px-8 py-8">
        <div className="mb-6">
          ...
          <p className="mt-0.5 text-[13px] text-text-muted">Start from a pre-built form or use one you saved</p>
        </div>
        ...
        {filteredUser.length > 0 && (
          <div className="mb-8">
            <h2 className="mb-3 text-[13px] font-semibold uppercase tracking-wider text-text-muted">My Templates</h2>
            <div className="grid grid-cols-3 gap-4">
              {filteredUser.map(t => (
                ...
                >
                  ...
                  {t.description && <p className="mt-1 text-[12px] text-text-muted">{t.description}</p>}
        ...
        {filteredBuiltin.length > 0 && (
          <div>
            {filteredUser.length > 0 && (
              <h2 className="mb-3 text-[13px] font-semibold uppercase tracking-wider text-text-muted">Built-in</h2>
            )}
            <div className="grid grid-cols-3 gap-4">
