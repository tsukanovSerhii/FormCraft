...
import { DowHeatmap, HourHeatmap } from '@/components/analytics/HeatmapChart'
...
export default function AnalyticsPage() {
	...
	const formAnalytics = selectedFormId === 'all' ? null : analyticsState.data
	const loadingAnalytics = selectedFormId !== 'all' && analyticsState.loading
	...
	useEffect(() => {
		...
		let cancelled = false
		analyticsApi
			.getFormAnalytics(selectedFormId, days)
			.then(data => {
				if (!cancelled) setAnalyticsState({ data, loading: false })
			})
			.catch(() => {
				if (!cancelled) setAnalyticsState({ data: null, loading: false })
			})
	...
	const stats = [
		{
			...
			delta: `${publishedForms} published`,
		...
		},
		{
			...
			delta: 'Published forms',
			deltaType: completionRate >= 50 ? ('up' as const) : ('down' as const),
			icon: CheckCircle2
		},
		{
			label: 'Avg per Form',
			value:
				totalForms > 0 ? String(Math.round(totalResponses / totalForms)) : '0',
			delta: 'responses / form',
			deltaType: 'up' as const,
			icon: TrendingUp
		}
	]
				...
				<div className="mb-6 flex items-start justify-between">
					<div>
						...
						<p className="mt-0.5 text-[13px] text-text-muted">
							Performance insights across your digital forms.
						</p>
					</div>
					...
					<div className="col-span-2 rounded-lg border border-border bg-surface p-5 shadow-card">
						<div className="mb-4">
							...
							<p className="text-[12px] text-text-muted">
								Daily aggregation · last {days} days
							</p>
						</div>
